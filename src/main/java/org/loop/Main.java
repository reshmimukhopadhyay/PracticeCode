package org.loop;

import org.loop.practiceLoop.*;

public class Main {
    public static void main(String[] args) {

        /*ForLoop1 fl=new ForLoop1();
        fl.loop1();*/

       /* ForLoop2 fl1=new ForLoop2();
        fl1.loop2();*/

        /*ForLoop3 fl2=new ForLoop3();
        fl2.loop3();*/

        /*ForLoop4 fl3=new ForLoop4();
        fl3.loop4();*/

        /*ForLoop5 fl4=new ForLoop5();
        fl4.loop5();*/

        /*ForLoop6 fl5=new ForLoop6();
        fl5.loop6();*/

        /*Loop7 fl6=new Loop7();
        fl6.loop7();*/

        /*ReverseNumber reverseNumber = new ReverseNumber();
        reverseNumber.reverseNumber();*/

        /*FibonacciSeries fs = new FibonacciSeries();
        fs.fibonacci();*/

        /*PrimeNumber pn = new PrimeNumber();
        pn.givenNumberIsprimeOrNot();*/

        /*IntegerPallindrom ip = new IntegerPallindrom();
        ip.integerPallindrom();*/

        /*StringPallindrom sp =new StringPallindrom();
        sp.stringPallindrom();*/

        /*ArmstrongNumber armstrongNumber=new ArmstrongNumber();
        armstrongNumber.armstrongNumber();*/

        /*ReverseString reverseString=new ReverseString();
        reverseString.reverseString();*/

        /*FactorialOfANumber factorialOfANumber = new FactorialOfANumber();
        factorialOfANumber.factorialOfANumberWithWhileLoop();
        factorialOfANumber.factorialOfANumberWithForLoop1();
        factorialOfANumber.factorialOfANumberWithForLoop2();*/

        NumberSorting numberSorting = new NumberSorting();
        //numberSorting.sortingProcessUsingTemporaryArray();
        numberSorting.sortingProcessUsingSingleArray();

    }




}